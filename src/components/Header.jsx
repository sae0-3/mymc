        ...
        <ul className='flex h-full gap-5'>
          {icons.map(({ icon, label, options }, idx) => (
            <li key={idx} className='relative group h-full'>
              <a
                href='#'
                className='text-white py-3 text-xl flex flex-col items-center hover:text-black group-hover:text-black duration-100'
              >
...
const icons = [
  ...
  {
    icon: 'tag-fill', label: 'Productos', options: [
  ...
  {
    icon: 'exclamation-triangle-fill', label: 'Reportes', options: [
